class ShutterCard extends HTMLElement {
    set hass(hass) {
        const _this = this;
        const topDownEntity = this.config.topDownEntity
        const bottomUpEntity = this.config.bottomUpEntity

        //Init the card
        if (!this.card) {
            const card = document.createElement("ha-card");
        
            if (this.config.title) {
                card.header = this.config.title;
            }

            this.card = card;
            this.appendChild(card);

            let allShutters = document.createElement("div");
            allShutters.className = "sc-shutters";


            /** UPDATES START HERE
             * Break out for each into entity selection of 
             * entity is bottom up entity and topdownEntity
             * 
             * Add two selectors one on top and one on bottom
             * Attribute math and postion to each individually
             */

            let entityId = bottomUpEntity;
            if (bottomUpEntity && bottomUpEntity.entity) {
              entityId = bottomUpEntity.entity;
            }

            let invertPercentage = false;
            if (this.config.invert_percentage) {
              invertPercentage = this.config.invert_percentage;
            }
            
            let partial = 0;
            if (this.config.partial_close_percentage) {
              partial = Math.max(0,Math.min(100,this.config.partial_close_percentage)); // make sure this is valid range
            }

            let offset = 0;
            if (this.config.offset_closed_percentage) {
              offset = Math.max(0,Math.min(100,this.config.offset_closed_percentage)); // make sure this is valid range
            }

            let tilt = false;
            if (this.config.can_tilt) {
              tilt = this.config.can_tilt;
            }

            let width = 153;
            if (this.config.shutter_width_px) {
              width = Math.max(10,this.config.shutter_width_px); // make sure this is valid range
            }

            let buttonsPosition = 'left';
            if (this.config.buttons_position) {
                buttonsPosition = this.config.buttons_position.toLowerCase();
                if (!['left', 'top', 'bottom', 'right'].includes(buttonsPosition)) {
                  buttonsPosition = 'left'
                }
            }
            const buttonsInRow = buttonsPosition == 'top' || buttonsPosition == 'bottom';
            const buttonsContainerReversed = buttonsPosition == 'bottom' || buttonsPosition == 'right';

            const titlePosition = this.config.titlePosition || "top";
            
            /*******
             * UI DOM ELEMENTS
             */
            let shutter = document.createElement("div");
            shutter.className = "sc-shutter";            
            shutter.dataset.shutter = entityId;
            shutter.innerHTML =
            `<div class="sc-shutter-top" ` + 
                (titlePosition == "bottom" ? 'style="display:none;"' : "") + `>
                <div class="sc-shutter-label"></div>
            </div>
            <div class="sc-shutter-middle" style="flex-flow: ` + (buttonsInRow ? 'column': 'row') + (buttonsContainerReversed ? '-reverse' : '') + ` nowrap;">
              <div class="sc-shutter-selector">
                <div class="sc-shutter-selector-picture" style="width: `+ width +`px">
                  <div class="sc-shutter-selector-td-picker"></div>  
                  <div class="sc-shutter-selector-slide"></div>
                  <div class="sc-shutter-selector-bu-picker"></div>
                  <div class="sc-shutter-movement-overlay">                
                    <ha-icon class="sc-shutter-movement-open" icon="mdi:arrow-up"></ha-icon>
                    <ha-icon class="sc-shutter-movement-close" icon="mdi:arrow-down"></ha-icon>
                  </div>
                </div>
              </div>
            </div>
            <div class="sc-shutter-bottom" ` + (titlePosition != 'bottom' ? 'style="display:none;"' : '') + `>
              <div class="sc-shutter-label"></div>
            </div>`;

            /*******
             * UI STYLE ELEMENTS
             */

            const style = document.createElement("style");
            style.textContent = `
                .sc-shutters { padding: 16px; }
                .sc-shutter { margin-top: 1rem; overflow: hidden; }
                .sc-shutter:first-child { margin-top: 0; }
                .sc-shutter-middle { display: flex; width: fit-content; max-width: 100%; margin: auto; }
                    .sc-shutter-buttons { flex: 1; text-align: center; margin-top: 0.4rem; display: flex; max-width: 100% }
                    .sc-shutter-buttons ha-icon-button { display: block; width: min-content }
                    .sc-shutter-selector { flex: 1; }
                    .sc-shutter-selector-partial { position: absolute; top:0; left: 9px; width: 88%; height: 1px; background-color: gray; }
                    .sc-shutter-selector-picture { position: relative; margin: auto; background-size: 100% 100%; min-height: 150px; max-height: 100%; }
                        .sc-shutter-selector-picture { background-image: url(data:@file/png;base64,); }
                    .sc-shutter-selector-slide { position: absolute; top: 19px; left: 6%; width: 88%; height: 0; }
                        .sc-shutter-selector-slide { background-image: url(data:@file/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAAGCAYAAAACEPQxAAAACXBIWXMAAA7EAAAOxAGVKw4bAAAAIGNIUk0AAHpPAACA1wAA+5gAAH6BAAB41AAA6bgAADhTAAAa8BY+DIIAAAAoSURBVHjaYjh48OBqpt+/f3sx8fHxcTFJSkoyMHFycjIwcXJyHgYMAKRuB6wLmIXlAAAAAElFTkSuQmCC); }
                    .sc-shutter-selector-td-picker { position: absolute; top: 19px; left: 6%; width: 88%; cursor: pointer; height: 7px; background-repeat: no-repeat; }
                        .sc-shutter-selector-td-picker { background-image: url(data:@file/png;base64,); }
                    
                    .sc-shutter-selector-bu-picker { position: absolute; top: 25px; left: 6%; width: 88%; cursor: pointer; height: 7px; background-repeat: no-repeat; }
                        .sc-shutter-selector-bu-picker { background-image: url(data:@file/png;base64,); }
                        
                    .sc-shutter-movement-overlay { 
                        position: absolute; top: 19px; left: 6%; width: 88%; height: 118px;
                        background-color: rgba(0,0,0,0.3); text-align: center; --mdc-icon-size: 60px
                    }
                        .sc-shutter-movement-open {display: none}
                        .sc-shutter-movement-close {display: none}
                .sc-shutter-top { text-align: center; margin-bottom: 1rem; }
                .sc-shutter-bottom { text-align: center; margin-top: 1rem; }
                    .sc-shutter-label { display: inline-block; font-size: 20px; vertical-align: middle; cursor: pointer;}
                    .sc-shutter-position { display: inline-block; vertical-align: middle; padding: 0 6px; margin-left: 1rem; border-radius: 2px; background-color: var(--secondary-background-color); }
            `;

            let picture = shutter.querySelector('.sc-shutter-selector-picture');
            let slide = shutter.querySelector('.sc-shutter-selector-slide');
            let td_picker = shutter.querySelector('.sc-shutter-selector-td-picker'); 
            let bu_picker = shutter.querySelector('.sc-shutter-selector-bu-picker');    
            
            let mouseDown = function(event) {
                if (event.cancelable) {
                  //Disable default drag event
                  event.preventDefault();
                }
                
                _this.isUpdating = true;
                _this.updatingElement = this;
                
                document.addEventListener('mousemove', mouseMove);
                document.addEventListener('touchmove', mouseMove);
                document.addEventListener('pointermove', mouseMove);
          
                document.addEventListener('mouseup', mouseUp);
                document.addEventListener('touchend', mouseUp);
                document.addEventListener('pointerup', mouseUp);
            };
      
            let mouseMove = function(event) {
              let newPosition = event.pageY - _this.getPictureTop(picture);
              if(_this.updatingElement.className == 'sc-shutter-selector-td-picker'){
                _this.setPickerPosition(newPosition, _this.updatingElement, this.td_minPosition, parseInt(bu_picker.style.top) - 7);
              } else {
                _this.setPickerPosition(newPosition, _this.updatingElement, parseInt(td_picker.style.top) + 7, this.bu_maxPosition);
              }
              
              _this.setSlideRealPosition(slide, td_picker, bu_picker)
            };
            
            let mouseUp = function(event) {
              _this.isUpdating = false;
              
              let minPosition = _this.td_minPosition;
              let maxPosition = _this.td_maxPosition;
              let entityId = td_entityId;
              let newPosition = parseInt(td_picker.style.top)
              if(_this.updatingElement.className == 'sc-shutter-selector-bu-picker') {
                minPosition = _this.bu_minPosition;
                maxPosition = _this.bu_maxPosition;
                entityId = bu_entityId;
                newPosition = parseInt(bu_picker.style.top)
              }

              if (newPosition < minPosition)
                newPosition = minPosition;
              
              if (newPosition > maxPosition)
                newPosition = maxPosition;
              
              let percentagePosition = (newPosition - minPosition) * (100-offset) / (maxPosition - minPosition);
              
              // if(_this.updatingElement.className == 'sc-shutter-selector-bu-picker') {
              //   _this.updateShutterPosition(hass, bu_entityId, parseInt(percentagePosition));
              // } else {
                _this.updateShutterPosition(hass, entityId, 100 - percentagePosition);
              //}
              
              _this.updatingElement = undefined;
              document.removeEventListener('mousemove', mouseMove);
              document.removeEventListener('touchmove', mouseMove);
              document.removeEventListener('pointermove', mouseMove);
          
              document.removeEventListener('mouseup', mouseUp);
              document.removeEventListener('touchend', mouseUp);
              document.removeEventListener('pointerup', mouseUp);
            };
          
            //Manage slider update
            td_picker.addEventListener('mousedown', mouseDown);
            td_picker.addEventListener('touchstart', mouseDown);
            td_picker.addEventListener('pointerdown', mouseDown);

            bu_picker.addEventListener('mousedown', mouseDown);
            bu_picker.addEventListener('touchstart', mouseDown);
            bu_picker.addEventListener('pointerdown', mouseDown);
        
            allShutters.appendChild(shutter);
            this.card.appendChild(allShutters);
            this.appendChild(style);
        }

        /******
         * UI JS 
         */

        let invertPercentage = this.config.invert_percentage || false;
        let offset = this.config.offset_closed_percentage || 0;
        let alwaysPercentage = false;
        let disableEnd = false;

        let td_entityId = topDownEntity;
        if (bottomUpEntity && topDownEntity.entity) {
          bu_entityId = topDownEntity.entity;
        }

        let bu_entityId = bottomUpEntity;
        if (bottomUpEntity && bottomUpEntity.entity) {
          bu_entityId = bottomUpEntity.entity;
        }

        const td_state = hass.states[td_entityId];
        const bu_state = hass.states[bu_entityId];

        //Update the shutters UI
        const shutter = _this.card.querySelector('div[data-shutter="' + bu_entityId +'"]');
        
        const friendlyName = (bottomUpEntity && bottomUpEntity.name) ? bottomUpEntity.name : bu_state ? bu_state.attributes.friendly_name : 'unknown';
        if(shutter){
          shutter.querySelectorAll(".sc-shutter-label").forEach(function (shutterLabel) {
              shutterLabel.innerHTML = friendlyName;
          });

          const td_currentPosition = td_state ? td_state.attributes.current_position : 'unknown';
          const td_movementState = td_state? td_state.state : 'unknown';
          const td_picker = shutter.querySelector('.sc-shutter-selector-td-picker');

          const bu_currentPosition = bu_state ? bu_state.attributes.current_position : 'unknown';
          const bu_movementState = bu_state? bu_state.state : 'unknown';
          const bu_picker = shutter.querySelector('.sc-shutter-selector-bu-picker');

          const slide = shutter.querySelector('.sc-shutter-selector-slide');
        
          if (!_this.isUpdating) {
            _this.setPickerPositionPercentage(td_currentPosition, td_picker, slide, invertPercentage, offset, this.td_minPosition, this.td_maxPosition);
            _this.setPickerPositionPercentage(bu_currentPosition, bu_picker, slide, invertPercentage, offset, this.bu_minPosition, this.bu_maxPosition);
            _this.setSlidePosition(td_currentPosition, bu_currentPosition, slide, invertPercentage, offset)
    
            _this.setMovement(td_movementState, bu_movementState, shutter);
          }
        }
    }

    /******
     * FUNCTIONS
     */

    changeButtonState(shutter, percent, inverted) {
        if (percent == 0) {
        shutter
          .querySelectorAll(".sc-shutter-button-up")
          .forEach(function (button) {
            button.disabled = inverted;
          });
        shutter
          .querySelectorAll(".sc-shutter-button-down")
          .forEach(function (button) {
            button.disabled = !inverted;
          });
      } else if (percent == 100) {
        shutter
          .querySelectorAll(".sc-shutter-button-up")
          .forEach(function (button) {
            button.disabled = !inverted;
          });
        shutter
          .querySelectorAll(".sc-shutter-button-down")
          .forEach(function (button) {
            button.disabled = inverted;
          });
      } else {
        shutter
          .querySelectorAll(".sc-shutter-button-up")
          .forEach(function (button) {
            button.disabled = false;
          });
        shutter
          .querySelectorAll(".sc-shutter-button-down")
          .forEach(function (button) {
            button.disabled = false;
          });
      }
    }
  
    positionPercentToText(percent, inverted, alwaysPercentage, hass) {
      if (!alwaysPercentage) {
        if (percent == 100) {
          return hass.localize(
            inverted
              ? "ui.components.logbook.messages.was_closed"
              : "ui.components.logbook.messages.was_opened"
          );
        } else if (percent == 0) {
          return hass.localize(
            inverted
              ? "ui.components.logbook.messages.was_opened"
              : "ui.components.logbook.messages.was_closed"
          );
        }
      }
      return percent + " %";
    }
  
    calculatePositionFromPercent(percent, inverted, offset, minPosition, maxPosition) {
      let visiblePosition;
      if (inverted) {
        visiblePosition = offset
          ? Math.min(100, Math.round((percent / offset) * 100))
          : percent;
      } else {
        visiblePosition = offset
          ? Math.max(0, Math.round(((percent - offset) / (100 - offset)) * 100))
          : percent;
      }
      return (
        ((maxPosition - minPosition) *
          (inverted ? visiblePosition : 100 - visiblePosition)) /
          100 +
        minPosition
      );
    }
  
    getPictureTop(picture) {
      let pictureBox = picture.getBoundingClientRect();
      let body = document.body;
      let docEl = document.documentElement;
  
      let scrollTop = window.pageYOffset || docEl.scrollTop || body.scrollTop;
  
      let clientTop = docEl.clientTop || body.clientTop || 0;
  
      let pictureTop = pictureBox.top + scrollTop - clientTop;
  
      return pictureTop;
    }
  
    setMovement(td_movement, bu_movement, shutter) {
      if (td_movement == "closing" || bu_movement == "closing") {
        shutter.querySelectorAll(".sc-shutter-movement-overlay").forEach((overlay) => (overlay.style.display = "block"));
        shutter.querySelectorAll(".sc-shutter-movement-close").forEach((overlay) => (overlay.style.display = "block"));
      } else if (td_movement == "opening" || bu_movement == "opening"){
        shutter.querySelectorAll(".sc-shutter-movement-overlay").forEach((overlay) => (overlay.style.display = "block"));
        shutter.querySelectorAll(".sc-shutter-movement-open").forEach((overlay) => (overlay.style.display = "block"));
      }
      else {
        shutter.querySelectorAll(".sc-shutter-movement-overlay").forEach((overlay) => (overlay.style.display = "none"));
        shutter.querySelectorAll(".sc-shutter-movement-close").forEach((overlay) => (overlay.style.display = "none"));
        shutter.querySelectorAll(".sc-shutter-movement-open").forEach((overlay) => (overlay.style.display = "none"));
      }
    }
  
    setPickerPositionPercentage(percentage, picker, slide, inverted, offset, minPosition, maxPosition) {
      let realPosition = this.calculatePositionFromPercent(
        percentage,
        inverted,
        offset,
        minPosition,
        maxPosition
      );
      
      this.setPickerPosition(realPosition, picker);
    }
  
    setPickerPosition(position, picker, min, max) {
      let minPosition = min || this.td_minPosition;
      let maxPosition = max || this.td_maxPosition;
      if(picker.className == 'sc-shutter-selector-bu-picker') {
        minPosition = min || this.bu_minPosition;
        maxPosition = max || this.bu_maxPosition;

      }
      if (position < minPosition) position = minPosition;
      if (position > maxPosition) position = maxPosition;
  
      picker.style.top = position + "px";
    }

    setSlideRealPosition(slide, td_picker, bu_picker){
      slide.style.top = (parseInt(td_picker.style.top, 10) + 7 ) + "px";
      slide.style.height = (parseInt(bu_picker.style.top, 10) - parseInt(slide.style.top, 10)) + "px";
    }

    setSlidePosition(top_position, bottom_position, slide, inverted, offset) {
      let realPositionTop = this.calculatePositionFromPercent(
        top_position,
        inverted,
        offset,
        this.td_minPosition,
        this.td_maxPosition
      );

      let realPositionBottom = this.calculatePositionFromPercent(
        bottom_position,
        inverted,
        offset,
        this.bu_minPosition,
        this.bu_maxPosition
      );
      
      realPositionTop = realPositionTop + 7;  //account for height of picker
      slide.style.top = realPositionTop + "px";
      slide.style.height = realPositionBottom - realPositionTop + "px";
    }
  
    updateShutterPosition(hass, entityId, position) {
      let shutterPosition = Math.round(position);
      hass.callService("cover", "set_cover_position", {
        entity_id: entityId,
        position: shutterPosition,
      });
    }
  
    setConfig(config) {
      if (!config.topDownEntity) {
        throw new Error("You need to define topDownEntity");
      }
      if (!config.bottomUpEntity) {
        throw new Error("You need to define bottomUpEntity");
      }

      this.td_maxPosition = 130;
      this.td_minPosition = 18;
      this.bu_maxPosition = 137;
      this.bu_minPosition = 25;
  
      this.config = config;
      this.isUpdating = false;
    }
  
    // The height of your card. Home Assistant uses this to automatically
    // distribute all cards over the available columns.
    getCardSize() {
      return 1;
    }
}

customElements.define("tdbu-shutter-card", ShutterCard);

